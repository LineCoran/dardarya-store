...
const PriceModal = () => {
  ...
  // цена выкупа
  const allPriceInChinaAlibaba = products.alibaba.reduce(
  ...
  const allDeliveryInChina = Number(
    products.alibaba.reduce((acc, item) => acc + Number(item.delivery), 0),
  );
  const allPriceInBLRWithOutPercent = Number(
    ((allPriceInChina + allDeliveryInChina) / 2).toFixed(2),
  );

  const allPriceInBYN =
    allPriceInBLRWithOutPercent > 400
      ? allPriceInBLRWithOutPercent.toFixed(2)
      : (allPriceInBLRWithOutPercent * 1.05).toFixed(2);
  ...
  const allPriceForDeliveryFromChinaToRB =
    allWeight > 50 ? (allWeight * 7.5 * usd).toFixed(2) : (allWeight * 7 * usd).toFixed(2);

  // доставка по РБ
  const allPriceForDeliveryBLR = priceForDeliveryInBLR(Number(allWeight));

  const finalPrice = (
    Number(allPriceInBYN) +
    Number(allPriceForDeliveryFromChinaToRB) +
    Number(allPriceForDeliveryBLR)
  ).toFixed(2);
  ...
  return (
    <div className={'price-modal-wrapper'}>
      ...
      <p>
        Доставка c Китая до РБ:{' '}
        <span style={{ fontWeight: '600' }}>{allPriceForDeliveryFromChinaToRB} BYN</span>
      </p>
      <p>
